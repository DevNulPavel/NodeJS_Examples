const PassThrough = require('stream').PassThrough; // Получаем только класс потока, способный пропускать через себя данные
...
const ytdl = (link, options) => {
    // Создаем промежуточный поток с нашими параметрами
    const stream = createStream(options);

    // Вызываем получение информации по ссылке
    ytdl.getInfo(link, options, (err, info) => {
        // Если ошибка, выбрасываем сообщение в поток
        if (err) {
            stream.emit('error', err);
            return;
        }
        
        // Загрузить данные в поток из полученной информации и параметров
...
// Сохраняем в экспортируемом объекте наши экспортируемые методы
...
const createStream = (options) => {
    // Создаем поток, который является прокси
    const stream = new PassThrough({
        highWaterMark: options && options.highWaterMark || null,
    });
    // Назначаем потоку обработчик разрушения, который выставляет флаг разрушения
    stream.destroy = () => { 
