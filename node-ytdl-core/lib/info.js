...
exports.getBasicInfo = (id, options, callback) => {
    // Пытаемся получить конфиг из страницы видео сначала
    const params = 'hl=' + (options.lang || 'en');
    let url = VIDEO_URL + id + '&' + params + '&bpctr=' + Math.ceil(Date.now() / 1000);
    
    // Убираем заголовочник, иначаче будет использоваться другой фреймворк для рендеринга контента
    ...
    request(url, reqOptions, (err, res, body) => {
        ...
        // Проверяем, есть ли ошибки в теле страницы
        const unavailableMsg = util.between(body, '<div id="player-unavailable"', '>');
        const rexExpr = /\bhid\b/;
        if (unavailableMsg && !rexExpr.test(util.between(unavailableMsg, 'class="', '"'))) {
            // Игнорируем ошибки об ограничении возраста
            if (!body.includes('<div id="watch7-player-age-gate-content"')) {
                let errorText = util.between(body, '<h1 id="unavailable-message" class="message">', '</h1>').trim();
                return callback(Error(errorText));
            }
        }
                
        // Парсим дополнительные метаданные из этой страницы
        const additional = {
            // Получаем автора из тела
            author: extras.getAuthor(body),
            
            // Получаем из текста дату публикации
...
const parseFormats = (info) => {
    ...
    // Удаляем из объекта info свойства
...
const gotConfig = (id, options, additional, config, fromEmbed, callback) => {
    // Если нет кофига - ошибка
    if (!config) {
        return callback(Error('Could not find player config'));
    }
    // Иначе пытаемся распарсить конфиг
    try {
    ...
    // Библиотека запросов, создаем URL запрос
    ...
    request(url, options.requestOptions, (err, res, body) => {
        ...
        if (!info.url_encoded_fmt_stream_map && !info.adaptive_fmts && !info.config && (config.args.fmt_list || config.args.url_encoded_fmt_stream_map || config.args.adaptive_fmts)) {
            info = config.args;
            info.no_embed_allowed = true;
        } else if (info.status === 'fail') {
            return callback( Error(`Code ${info.errorcode}: ${util.stripHTML(info.reason)}`));
        }
        ...
        try {
            ...
            return callback(Error('Error parsing `player_response`: ' + err.message));
        }
